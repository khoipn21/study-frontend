...
class Media {
  ...
  createShader() {
    ...
    img.onload = () => {
      texture.image = img
      ;(this.program.uniforms.uImageSizes as { value: Array<number> }).value = [
        img.naturalWidth,
        img.naturalHeight,
      ]
    }
  ...
  onResize({
    ...
    ;(
      this.plane.program.uniforms.uPlaneSizes as { value: Array<number> }
    ).value = [this.plane.scale.x, this.plane.scale.y]
    this.padding = 2
