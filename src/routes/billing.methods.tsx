...
function PaymentMethodsPage() {
  ...
  const methods = useQuery({
    ...
    queryFn: async () =>
      (await api.listPaymentMethods(token || '')).payment_methods ?? [],
    enabled: !!token,
  })

  const create = useMutation({
    mutationFn: async (payload: any) =>
      api.createPaymentMethod(token || '', payload),
  ...
  const setDefault = useMutation({
    mutationFn: async (id: string) =>
      api.setDefaultPaymentMethod(token || '', id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['billing', 'methods'] }),
  })
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-2 mb-4">
        ...
        <button
          className="px-3 py-2 bg-blue-600 text-white rounded self-start"
        ...
        {methods.data?.map((m: any) => (
          <li key={m.id} className="p-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="text-red-600" onClick={() => del.mutate(m.id)}>
                Delete
              </button>
            </div>
          </li>
